...
| 负重 | 尚无物品重量字段与计算能力；无效的 `showEncumbrance` 偏好已移除，未来需先补内容数据再设计完整流程 |
...
- 作者 GUI（资料包设置页「我的自制内容」）：新建 / 编辑 / 删除走
  ...
  来源；职业资源是**补丁声明**——表单新增的资源只有 `id`，必须补上名称与上限才能导出
  （否则导入期 `incompleteResourcePatch`）；「基于现有条目创建覆盖」把新条目的**对齐键**
  （id 末段）钉在来源条目上，未声明的列仍继承来源，但**不预填来源的 `rules`**
  （来源 grants / choices 常引用来源包内的条目，带进本地包会让这个包再也导不出去）；
  覆盖链只消费 `classRules`，条目自己的 `rules` 只在角色直接指向该条目时生效；
  重名会自动加 `-2`/`-3` 后缀，**停用自制包也不例外**；「导出 .dndpack」先用真实导入器
  自校验再落盘，并把组合仓库加的 `local:` / `campaign:<cid>:` 传输前缀从条目 id、
  关系目标与 `rules` 引用上剥掉（前缀不属于包格式）——**读与写两个方向都剥**：
  编辑保存后本地库仍是规范 id，否则再读一层前缀就会让该包永远导不出去。编辑既有条目时
  描述框之外的块（标题 / 列表等）原样保留。
...
自制条目更新时，未传入的摘要、正文、`structured`、标签和 `rules` 保留原值；
显式传入的 `structured` 是完整替换，空对象可清空，避免 JSON 编辑器删除的字段在保存后复活。
显式空摘要、空描述、空标签与空 `rules` 分别清空对应内容；正文未传入时原样保留全部块。
2026-09-23 已补设置页编辑 → `.dndpack` 导出 → 真实导入器 → 建卡的往返回归；
