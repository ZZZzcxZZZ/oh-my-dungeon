...
2026-07-29 已完成批次一至批次六：

- 私人工作区、服务器稳定身份、账号隔离和旧数据迁移已落地；
- 玩家加入、角色发布与绑定、DM 战役角色目录和档案权限已收敛；
- 战役失效协调、私聊/小群显示、未读刷新和折叠战役卡片已完成；
- 资料阅读、紧凑筛选、长旁白、头像工具条和共享 Material 3 组件已统一；
- Character Markdown v2、自动文件镜像、外部修改差异确认、怪物动作投影已完成；
- 旧 Actor 持久化命名已无损迁移为 Character，未实现的战斗入口已隐藏。

最终验证：

- Flutter 全量测试：845 通过，2 个依赖私有路径的测试按设计跳过；
- Flutter analyze：零问题；
- 服务端测试：352 通过（27 套件），lint、build、Prisma schema 校验通过；
- PHB 私有包：1106 条、零断链，真实 Flutter 导入和全部子职关联验证通过；
- 私有提取器：PHB/MM/DMG 相关测试通过；
- 私有 release Web：PHB/MM/DMG 共 1880 条成功注入并构建，构建后公开占位文件自动恢复；
- 桌面和 390×844 移动视口启动无控制台错误、无横向溢出，资料库可实际打开并读取内置条目。
