...
void main() {
  ...
  test('uses credentials that become available after construction', () async {
    ...
    expect(
      apiClient.createEntryCalls.single['accessToken'],
      'token-after-login',
    );
    controller.dispose();
  });
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.enterText(find.byKey(const Key('editor-field-name')), '黑塔');
  ...
  testWidgets('json import dialog parses array and previews count', (
    tester,
  ) async {
    final cacheRepository = MemoryCampaignCacheRepository();
    final apiClient = MemoryCampaignSyncApiClient();
    ...
    expect(
      find.byKey(const Key('campaign-json-import-dialog')),
      findsOneWidget,
    );
  ...
  testWidgets('delete entry calls api and removes from cache', (tester) async {
    final cacheRepository = MemoryCampaignCacheRepository(
      entries: [testContentEntry(id: 'entry-1', name: '月港')],
    );
    final apiClient = MemoryCampaignSyncApiClient();
    final controller = CampaignContentController(
      cacheRepository: cacheRepository,
  ...
  testWidgets('create failure shows error and does not fake success', (
    tester,
  ) async {
    final cacheRepository = MemoryCampaignCacheRepository();
    ...
    await tester.enterText(find.byKey(const Key('editor-field-name')), '失败条目');
    await tester.tap(find.widgetWithText(FilledButton, '保存'));
    await tester.pumpAndSettle();
  ...
  testWidgets('player mode hides edit actions', (tester) async {
    final cacheRepository = MemoryCampaignCacheRepository(
      entries: [testContentEntry(id: 'entry-1', name: '月港')],
    );
    final apiClient = MemoryCampaignSyncApiClient();
