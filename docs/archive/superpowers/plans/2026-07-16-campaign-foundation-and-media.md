...
**架构：** 保留既有 `Campaign`、`CampaignMember`、`CampaignCharacter` 和 typed chat message 体系；扩展成员状态和角色生命周期，集中到 `CampaignPolicy` 做能力判定。消息创建始终由服务端解析当前成员身份并生成不可变展示快照，客户端只提交意图。媒体在此批次仅建立头像资产元数据与受控上传接口，文件实现沿用 Nest 的本地部署约定。
...
1. 先在 e2e 测试新增断言：创建者成员角色为 `owner`；成员视图含 `boundCharacterId`、`activeSpeakerCharacterId`、`lastReadAt`（未设置时为 `null`）。
...
4. 为 `CampaignMember` 加 `boundCharacterId`、`activeSpeakerCharacterId`、`speakerMode`（默认 `boundCharacter`）、`lastReadAt`；建立与 `CampaignCharacter` 的命名关系。
5. 为 `CampaignCharacter` 加 `lifecycle`（persistent/temporary/archived）、`avatarAssetId`、`healthVisibility`，保留兼容的 `status`。
...
- 修改：`apps/server_nest/src/modules/campaign-sync/campaign-characters.service.ts`
...
1. 新增失败用例：玩家不能创建 NPC、不能编辑其他玩家 character；DM 可以管理任意 character；归档 character 不能作为发言身份；玩家只能拥有一条 bound character。
2. 运行 `npm run test:server -- campaign.policy.spec.ts campaign-characters.e2e-spec.ts`，确认红灯来自未实现的能力限制。
3. 在 `CampaignPolicy` 增加 `capabilitiesFor`、`canBindCharacter`、`canSpeakAsCharacter`、`canManageMembershipBinding`，并把 owner 统一视为 `owner` 角色而非仅依赖 `Campaign.ownerId` 的特例。
4. 在 character service 中提供最小 character 摘要投影：身份、头像资产、公开生命状态、生命周期、是否可作为当前用户发言身份；禁止玩家读到其他角色的精确 HP/资源。
5. 在所有写路径使用新的 policy，不保留“前端传 characterId 就能冒充”的分支。
...
1. 为以下行为先写 e2e 红灯测试：
   - 玩家能绑定自己发布的一个 character，第二次绑定被拒绝。
   - DM 能为任意成员绑定或清空角色；普通玩家不能改别人绑定。
   - 玩家没有绑定时只能发 `ooc`，`say`/`action` 被拒绝。
   - 当前 speaker 跨请求保存，DM 可选 narrator/NPC/临时 character，玩家只能选 bound character 或 OOC。
   - `GET /campaigns/:id/context` 返回 viewer、capabilities、members、characters、当前 membership 和 unread 计数。
...
4. 当 DM 以“快速临时身份”发送第一条消息时，在 transaction 内创建临时 character、写入 active speaker、写消息；失败时三者都不落库。
5. 消息读模型返回 immutable speaker snapshot 和 health projection，绝不直接下发其他玩家完整 sheet。
6. 运行 e2e 测试至绿，随后运行 `npm run test:server -- campaigns.e2e-spec.ts campaign-characters.e2e-spec.ts`。
...
5. gateway 只广播经过 viewer projection 的消息，不将 character 私有 sheet 投给 socket 房间。
...
4. 提供“附着头像”的 service 方法，分别用于用户、campaign、campaign character；只接受同一 campaign 中授权的 character。
