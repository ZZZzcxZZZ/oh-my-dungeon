...
**架构：** 服务端成员角色和能力集继续作为权限权威；客户端以 `CampaignActorController` 为完整 Actor 唯一状态源，并通过一个工作区刷新协调器让聊天上下文与 Actor 缓存同步。战役中心重新分为概览、角色、档案、记录，所有角色入口复用同一完整角色卡。私人资料使用单一 UTF-8 构建入口，在 Flutter 启动门槛中完成内容哈希安装和子职业端到端校验。
...
- 创建 `apps/client_flutter/lib/src/features/campaigns/presentation/campaign_workspace_mutation_coordinator.dart`：统一刷新 Actor 缓存和工作区上下文。
...
- 创建 `apps/client_flutter/lib/src/features/campaigns/presentation/actors/campaign_actor_sheet_launcher.dart`：统一角色卡权限计算和页面导航。
- 修改 `apps/client_flutter/lib/src/features/campaigns/presentation/actors/campaign_actor_sheet_page.dart`：支持可编辑和只读两种状态。
...
- 修改 `apps/server_nest/src/modules/campaigns/policies/campaign.policy.ts`：返回邀请、Actor 编辑和档案能力。
...
- 修改 `apps/client_flutter/test/campaign_actor_pages_test.dart`
...
- 测试：`apps/client_flutter/test/campaign_chat_actor_test.dart`
...
```powershell
git diff --check
rg -n "showModalBottomSheet|CampaignActorQuickSheet|speakerMode|boundActor" apps/client_flutter/lib/src/features/campaigns/presentation/campaign_chat_page.dart
```
...
```powershell
flutter test test/campaign_chat_actor_test.dart
```
...
```powershell
git add apps/client_flutter/lib/src/features/campaigns/presentation/campaign_chat_page.dart apps/client_flutter/lib/src/features/campaigns/presentation/campaign_controller.dart apps/client_flutter/lib/src/features/campaigns/presentation/campaigns_tab_page.dart apps/client_flutter/lib/src/features/campaigns/presentation/chat apps/client_flutter/test/campaign_chat_actor_test.dart
git commit -m "refactor(v0.1): simplify campaign chat composer"
```

### 任务 2：统一 DM 身份和 Actor 同步
...
测试应构造记录调用次数的 fake controller，并断言一次工作区变更同时刷新 Actor 和 workspace：

```dart
test('refreshAfterActorMutation refreshes actors before workspace context', () async {
  final calls = <String>[];
...
运行 `flutter test test/campaign_chat_actor_test.dart`，预期 DM 无主角色断言失败。
...
```dart
final controllableActors = actors.where((actor) {
  if (!canManageCampaign || actor.status != 'active') return false;
...
```dart
await mutationCoordinator.refreshAfterActorMutation(campaign.id);
```
...
```powershell
flutter test test/campaign_workspace_mutation_coordinator_test.dart test/campaign_chat_actor_test.dart
flutter analyze
git add apps/client_flutter/lib/src/features/campaigns/presentation/campaign_workspace_mutation_coordinator.dart apps/client_flutter/lib/src/features/campaigns/presentation/campaign_chat_page.dart apps/client_flutter/lib/src/features/campaigns/presentation/chat/campaign_identity_sheet.dart apps/client_flutter/lib/src/features/campaigns/presentation/chat/chat_mode_picker.dart apps/client_flutter/test/campaign_workspace_mutation_coordinator_test.dart apps/client_flutter/test/campaign_chat_actor_test.dart
git commit -m "fix(v0.1): unify campaign speaker and actor refresh"
```
...
```dart
String campaignActorSection(CampaignActor actor) {
  if (actor.status == 'archived') return 'archived';
  if (actor.lifecycle == 'temporary') return 'temporary';
  if (actor.actorType == 'player') return 'player';
  if (actor.actorType == 'npc') return 'npc';
  return 'other';
}
```
...
**文件：**
- 创建：`apps/client_flutter/lib/src/features/campaigns/presentation/actors/campaign_actor_sheet_launcher.dart`
- 修改：`apps/client_flutter/lib/src/features/campaigns/presentation/actors/campaign_actor_sheet_page.dart`
- 修改：`apps/client_flutter/lib/src/features/campaigns/presentation/campaign_chat_page.dart`
- 修改：`apps/client_flutter/lib/src/features/campaigns/presentation/center/campaign_characters_panel.dart`
- 修改：`apps/client_flutter/lib/src/features/campaigns/presentation/center/campaign_overview_panel.dart`
- 测试：`apps/client_flutter/test/campaign_actor_pages_test.dart`
- 测试：`apps/client_flutter/test/campaign_chat_actor_test.dart`

- [ ] **步骤 1：为权限函数编写失败测试**

```dart
expect(canEditCampaignActor(actor: ownActor, currentUserId: 'u1', canEditAnyActor: false), isTrue);
expect(canEditCampaignActor(actor: otherActor, currentUserId: 'u1', canEditAnyActor: false), isFalse);
expect(canEditCampaignActor(actor: otherActor, currentUserId: 'u1', canEditAnyActor: true), isTrue);
```

运行 `flutter test test/campaign_actor_pages_test.dart`，预期 FAIL，权限函数尚不存在。
...
}) => canEditAnyActor || actor.ownerUserId == currentUserId;
```

`openCampaignActorSheet` 根据该结果构建 `CampaignActorSheetPage(canEdit: canEdit)`。
...
`CampaignActorSheetPage` 新增：

```dart
final bool canEdit;
```

所有写操作仅在 `canEdit && actor.status != 'archived'` 时构建。只读状态仍显示完整属性、资源、特性、法术、装备和备注内容。

- [ ] **步骤 5：迁移所有入口**

聊天消息头像、输入栏头像、角色目录条目和概览成员绑定角色都调用统一 launcher。移除这些入口对 `CampaignActorQuickSheet` 的使用。

- [ ] **步骤 6：验证并提交**

```powershell
flutter test test/campaign_actor_pages_test.dart test/campaign_chat_actor_test.dart test/campaign_center_page_test.dart
flutter analyze
git add apps/client_flutter/lib/src/features/campaigns/presentation/actors apps/client_flutter/lib/src/features/campaigns/presentation/campaign_chat_page.dart apps/client_flutter/lib/src/features/campaigns/presentation/center apps/client_flutter/test/campaign_actor_pages_test.dart apps/client_flutter/test/campaign_chat_actor_test.dart apps/client_flutter/test/campaign_center_page_test.dart
...
- 测试：`apps/server_nest/test/campaign-actors.e2e-spec.ts`
...
服务返回 `apiVersion: '1'` 和 `features: ['campaignArchives', 'campaignActors', 'campaignChat']`。
...
```powershell
npm run test:server -- --runInBand src/modules/campaigns/policies/campaign.policy.spec.ts test/server-info.e2e-spec.ts test/campaign-actors.e2e-spec.ts
npm run lint:server
...
```powershell
flutter test test/campaign_workspace_mutation_coordinator_test.dart test/campaign_chat_actor_test.dart test/campaign_center_page_test.dart test/campaign_characters_panel_test.dart test/campaign_actor_pages_test.dart test/campaign_archive_panel_test.dart test/bundled_content_installer_test.dart test/tooling/private_content_package_validation_test.dart
flutter analyze
```
...
```powershell
npm run test:server -- --runInBand src/modules/campaigns/policies/campaign.policy.spec.ts test/campaigns.e2e-spec.ts test/campaign-actors.e2e-spec.ts test/server-info.e2e-spec.ts
npm run lint:server
```
...
- 类型一致：统一使用 `canEditAnyActor`、`CampaignWorkspaceMutationCoordinator.refreshAfterActorMutation` 和 `openCampaignActorSheet`。
