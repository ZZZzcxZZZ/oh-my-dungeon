...
8. **选择系统（§3.10）**：字符串简写自动授予且选中生效；内联选项的 `ability`/`hitPoints` 改变派生结果；`optionType: "spell"` + `countsToward` 受 `prepared` 约束；`repeatable` 行为正确；`requires` 隐藏语义正确；`group`/`help` 呈现；装备 A/B 方案写入 `inventory`；技能选择与法术选择都只由 `optionType: "skill"` / `"spell"` 的选择定义承担（`classRules` 不再派生选择）。
   ...
   **边界**：PHB 提取器尚未产出 `optionType: "spell"` 的选择（运行时已支持，见 §11）；
   背景技能由**背景条目自己的 `rules`** 驱动（决策 D10，已落地）。
...
本节是"什么做了 / 什么没做 / 为什么"的**唯一记录**。没做的项都带理由与"何时重新评估"，
不允许出现"说不清为什么没做"的空白。
...
| 能力 | 落地位置 |
|---|---|
| 选择系统运行时语义（`grants` 简写自动授予、`repeatable`、`countsToward`、`requires`、`group` / `help`、装备 A/B、`invalidAutoGrant`） | §3.10；计划 2 |
| 列级合并、列级来源被消费、`mode: patch/replace`、`priority` + Drift 13→14、冲突提示与改选、关闭覆盖回退 | §3.6–§3.8；S3 |
...
| 能力 | 决策 | 理由 | 何时重新评估 |
|---|---|---|---|
| 资源池的**具体效果**（引导神力选项、野性形态数据与形态切换、术法点转换法术位、圣疗治疗结算、魔法诡计恢复法术位、诗人激励骰的授予与消耗） | 不建模：只追踪"用了几次 / 怎么恢复" | 效果是逐条特性的自由文本 + 各自独立的子规则（形态数据表、点数转换、治疗池）。造一个"通用效果"模型等于在契约里再写一套规则引擎，与"内容只描述数值与枚举"冲突 | 有真实内容包需要**自动结算**这些效果时，按逐条特性立项 |
| 伤害抗性与免疫 | 不建模 | 需要"伤害类型"贯通攻击 / 法术 / 环境三条结算路径，并定义抗性来源（种族 / 职业 / 物品 / 状态）与叠加规则；改动面在结算层，不是契约层 | 需要自动减半 / 免伤时 |
| 奥法骑士 / 诡术师的准备或已知上限与戏法上限 | **刻意不建模** | 2024 官方职业表**没有给出**这两列；按 2014 公式臆造会产出错误上限（本项目禁止把 2014 公式当 2024 规则） | 官方表可查时 |
| 命中骰（Hit Dice）池 | 不建模 | 需要"骰池 + 短休消耗 + 长休回半"的数据与 UI；HP 恢复本身已可用，消耗 HD 只是**可选**途径 | 需要"短休消耗 HD 回血"时 |
| 力竭等级惩罚 | 不建模 | 力竭现为普通条件计数；自动惩罚要先定义"等级的来源与叠加顺序"（多个来源同时施加时） | 需要自动施加 −2×等级 / −5×等级尺速度时 |
| 专注校验 | 不建模 | 需要"同时只维持一个 + 受伤时 DC 检定"的**运行时钩子**；当前只有 `concentrating` 条件预设 | 战斗 / 施法运行时立项时 |
...
| **可视化规则表单** | 已实现（`HomebrewClassRuleForm`：`classRules` 的 `hitDie` / `savingThrowAbilities` / `mode` / `spellcasting.ability` / `resources[]`，`rules.progression[]` 的等级集合与 `grants[]`）。表单**不持有状态**：两段 JSON 仍是唯一事实来源，JSON 非法时表单拒绝渲染并提示切回 JSON；`Table` / `MaxSpec` 形态与 `rules.choices` 不伪造字段类型，仍写 JSON（表形态的 `recovery` 只读展示，不提供可一键替换成常量的下拉）。"添加资源"不写 `name`（缺省 = patch 未声明，合法），不产出 `name: ""` 这种导出期才报错的中间态；自由文本不 trim、令牌字段 trim |
...
> 本表是 2026-09-10 定稿时的阶段划分，**已全部完成**（见 §10 验收标准与 §11.1）。
> 保留它是为了追溯"先定契约、再重写提取器、最后搬迁数值"的顺序与理由，不是待办。
