...
**架构：** 主壳继续用 `IndexedStack` 保持四个一级页面状态；聊天模式控件保留现有枚举和回调；消息分组偏好持久化到客户端设置；聊天头像从当前 `CampaignCharacter` 投影生命值，角色写入成功后控制器立即刷新内存状态。
...
```powershell
flutter test test/campaign_chat_composer_test.dart test/campaign_chat_character_test.dart
```
...
```powershell
flutter test test/offline_shell_test.dart test/widget_test.dart test/campaign_chat_composer_test.dart test/campaign_chat_character_test.dart test/campaign_characters_panel_test.dart
flutter analyze
```
