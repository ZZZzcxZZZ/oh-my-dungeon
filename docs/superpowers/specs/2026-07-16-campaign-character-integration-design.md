...
- The owner can view and edit every campaign actor.
- A player can edit only a campaign actor owned by that player.
...
1. Campaign hub: information, members and actors, shared content, and owner
   controls.
...
The member list uses campaign actors rather than private local characters.
Selecting an actor opens a quick sheet. The campaign owner may choose an
ability, saving throw, or skill and send a check request to that actor.
...
- `say`: in-character speech with actor identity and a speech surface.
- `action`: in-character action with actor identity and italic body text.
- `roll`: a dice result with actor identity and structured roll metadata.
- `system`: a bold campaign event such as item gain or rest completion.
- `checkRequest`: a DM request targeting an actor and a named check.

Every actor-originated event stores server-derived display name and avatar
...
The server validates message kind, actor ownership, campaign management rights,
target actor membership, supported check type, DC range, and roll mode.
...
- Action messages visibly identify their actor.
- The owner can request a player actor check and the player can answer it.
