...
import 'package:dnd_table_client/src/features/campaigns/presentation/characters/campaign_character_directory_page.dart';
import 'package:dnd_table_client/src/features/campaigns/presentation/characters/campaign_character_sheet_launcher.dart';
import 'package:dnd_table_client/src/features/campaigns/presentation/characters/campaign_character_sheet_page.dart';
...
void main() {
  ...
  test(
    'campaign character edit permission follows ownership and DM authority',
    () {
      final ownCharacter = testCampaignCharacter(
        id: 'own',
        ownerUserId: 'user-1',
        sheet: const {'name': 'Own'},
      );
      final otherCharacter = testCampaignCharacter(
        id: 'other',
        ownerUserId: 'user-2',
  ...
  testWidgets('read-only character sheet hides every editing affordance', (
    tester,
  ) async {
    final controller = CampaignCharacterController(
      cacheRepository: MemoryCampaignCacheRepository(
        characters: [_sampleCampaignCharacter],
      ),
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CampaignCharacterSheetPage(
          controller: controller,
          characterId: _sampleCampaignCharacter.id,
          canEdit: false,
        ),
      ),
    );
    ...
    expect(find.byKey(const Key('campaign-character-sheet')), findsOneWidget);
    expect(find.byTooltip('归档'), findsNothing);
    expect(
      find.byKey(const Key('campaign-character-avatar-picker')),
      findsNothing,
    );
  ...
  test('uses credentials that become available after construction', () async {
    ...
    final published = await controller.publishCharacter(_sampleLocalCharacter);
  ...
  // characters via the DM create endpoint (/characters), not the self-publish endpoint
  // (/characters/publish) which rejects non-player types. See
  // docs/superpowers/plans/2026-07-17-workspace-spec-compliance-fixes.md Task 1.3.
  test(
    'createDmCharacter creates a persistent NPC via the DM create endpoint',
    () async {
      final apiClient = MemoryCampaignSyncApiClient();
      final controller = CampaignCharacterController(
        cacheRepository: MemoryCampaignCacheRepository(),
      ...
      final created = await controller.createDmCharacter(
        characterType: 'npc',
  ...
  test(
    'createDmCharacter creates a persistent monster via the DM create endpoint',
    () async {
      final apiClient = MemoryCampaignSyncApiClient();
      final controller = CampaignCharacterController(
        cacheRepository: MemoryCampaignCacheRepository(),
  ...
  // "Only player characters can be self-published; use the DM create endpoint".
  test('publishCharacter surfaces server error message on 400', () async {
    ...
    expect(
      controller.error,
      contains('Only player characters can be self-published'),
    );
    controller.dispose();
  });
  ...
  test(
    'publishCharacter uses local character revision as baseRevision when re-publishing',
    () async {
  ...
  // Spec §双向同步 切片 A: 发布成功后立即调用 backlink 回调把远端 character
  // 回写本地角色，避免等下次 pullUntilCurrent 才同步运行时字段。
  test(
    'publishCharacter invokes onCharacterPublished callback after success',
    () async {
      final apiClient = MemoryCampaignSyncApiClient();
      CampaignCharacter? publishedCharacter;
      final controller = CampaignCharacterController(
        cacheRepository: MemoryCampaignCacheRepository(),
        apiClient: apiClient,
        apiBaseUrl: 'https://example.test',
        accessToken: 'access-token',
        currentUserId: 'user-1',
        onCharacterPublished: (character) async {
      ...
      expect(published, isTrue);
      expect(publishedCharacter, isNotNull);
      expect(publishedCharacter!.sourceCharacterId, 'char-1');
      controller.dispose();
    },
  );

  testWidgets('player and dm modes keep their character libraries isolated', (
    tester,
  ) async {
    final characterRepository = MemoryCharacterRepository(
      initial: [_sampleLocalCharacter],
    );
    ...
    final cacheRepository = MemoryCampaignCacheRepository(
      characters: [_sampleCampaignCharacter, _sampleNpc, _archivedCharacter],
    );
    final apiClient = MemoryCampaignSyncApiClient();
    ...
    await drainStream();
    await campaignCharacterController.selectCampaign('campaign-1');
    await drainStream();
    ...
    // Player mode only manages private local characters. Binding is handled
    // when entering a campaign, so the local card never exposes publication.
    ...
    expect(find.byKey(const Key('campaign-character-directory')), findsNothing);
  ...
  testWidgets(
    'dm mode character directory supports search and status filters',
    (tester) async {
      final cacheRepository = MemoryCampaignCacheRepository(
        characters: [_sampleCampaignCharacter, _sampleNpc, _archivedCharacter],
      );
      ...
      await drainStream();
      await characterController.selectCampaign('campaign-1');
      await drainStream();

      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: CampaignCharacterDirectoryPage(
      ...
      await tester.enterText(
        find.byKey(const Key('character-directory-search')),
        'Goblin',
      );
      await tester.pumpAndSettle();
      expect(find.text('Test Hero'), findsNothing);
  ...
  testWidgets('dm opens character sheet and updates hp with base revision', (
    tester,
  ) async {
    final cacheRepository = MemoryCampaignCacheRepository(
      characters: [_sampleCampaignCharacter],
    ...
    expect(
      find.byKey(const Key('campaign-character-full-sheet')),
      findsOneWidget,
    );
    ...
    expect(apiClient.updateCharacterCalls, hasLength(1));
    expect(apiClient.updateCharacterCalls.last['baseRevision'], 1);
    expect(apiClient.updateCharacterCalls.last['characterId'], 'character-1');
    final sheet =
        apiClient.updateCharacterCalls.last['sheet']! as Map<String, Object?>;
  ...
  testWidgets('publish character sheet submits to api with base revision 0', (
      ...
      MaterialApp(
        home: Scaffold(
          body: PublishCharacterSheet(
            controller: campaignCharacterController,
            character: _sampleLocalCharacter,
          ),
        ),
      ),
    );
    ...
    expect(apiClient.publishCalls.last['characterType'], 'player');
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final apiClient = MemoryCampaignSyncApiClient();
      apiClient.nextPublishCharacterException = const CampaignConflictException(
        {
          'id': 'character-existing',
          'campaignId': 'campaign-1',
      ...
      // "用本地覆盖"：用服务端 revision=7 重试 publishCharacter。
  ...
  testWidgets('character update conflict shows comparison and reload action', (
    tester,
  ) async {
    final cacheRepository = MemoryCampaignCacheRepository(
      characters: [_sampleCampaignCharacter],
    ...
    final apiClient = MemoryCampaignSyncApiClient();
    apiClient.nextUpdateCharacterException = const CampaignConflictException({
      'id': 'character-1',
      'campaignId': 'campaign-1',
      'characterType': 'player',
      'status': 'active',
    ...
    expect(find.byKey(const Key('character-conflict-dialog')), findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('character-conflict-dialog')), findsNothing);

    characterController.dispose();
  });

  testWidgets('dm character sheet shows runtime ledger and edit history', (
    tester,
  ) async {
    final character = testCampaignCharacter(
      id: 'character-audit',
      campaignId: 'campaign-1',
      sheet: const {
        'name': 'Audit Hero',
    ...
    final apiClient = MemoryCampaignSyncApiClient(
      characterAudits: const [
        CampaignCharacterAudit(
          id: 'audit-1',
          campaignCharacterId: 'character-audit',
          campaignId: 'campaign-1',
    ...
    final controller = CampaignCharacterController(
      cacheRepository: MemoryCampaignCacheRepository(characters: [character]),
      apiClient: apiClient,
      apiBaseUrl: 'https://example.test',
      accessToken: 'access-token',
    ...
    expect(apiClient.listCharacterAuditCalls, ['character-audit']);

    controller.dispose();
  });
}

const _sampleLocalCharacter = CharacterSheet(
  id: 'char-1',
  ownerUserId: 'user-1',
  name: 'Mira',
  avatarUrl: null,
...
final CampaignCharacter _sampleCampaignCharacter = testCampaignCharacter(
  id: 'character-1',
  campaignId: 'campaign-1',
  ownerUserId: 'user-1',
  sourceCharacterId: 'char-1',
...
final CampaignCharacter _sampleNpc = testCampaignCharacter(
  id: 'character-2',
  campaignId: 'campaign-1',
  ownerUserId: null,
...
final CampaignCharacter _archivedCharacter = testCampaignCharacter(
  id: 'character-3',
  campaignId: 'campaign-1',
  ownerUserId: null,
  sourceCharacterId: null,
