...
///
/// **唯一实现点**：引擎自计划任务 3 起已直接调用本层的
/// [normalizeSelection] / [requiresSatisfied] / [grantsForSelection]（经
/// [candidatesFor] 复用候选枚举）；三处选择 UI（创建向导 / 编辑器升级队列 /
/// 独立升级页）的直接调用在计划任务 6+ 迁移，迁移完成后调用方不得再写第二套候选 /
/// 规范化 / 授予判断。
///
...
abstract final class RuleChoiceSemantics {
  ...
  ///
  /// **唯一实现点**：引擎自计划任务 3 起经 [normalizeSelection] /
  /// [grantsForSelection] 间接调用；三处 UI 的直接调用在计划任务 6+ 迁移。
  ///
  ...
  ///
  /// **唯一实现点**：引擎自计划任务 3 起直接调用；三处 UI 的直接调用在计划任务
  /// 6+ 迁移。
  ///
  /// 规则（顺序即用户选择顺序，**不排序**）：
