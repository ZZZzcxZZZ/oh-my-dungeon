...
- 作者 GUI（资料包设置页「我的自制内容」）：新建 / 编辑 / 删除走
  ...
  未声明的列仍继承来源，但**不预填来源的 `rules`**（来源 grants / choices 常引用来源包内的
  条目，带进本地包会让这个包再也导不出去）；覆盖链只消费 `classRules`，条目自己的
  `rules` 只在角色直接指向该条目时生效；重名会自动加 `-2`/`-3` 后缀，**停用自制包也不
  例外**；「导出 .dndpack」先用真实导入器自校验再落盘，并把组合仓库加的
  `local:` / `campaign:<cid>:` 传输前缀从条目 id、关系目标与 `rules` 引用上剥掉
  （前缀不属于包格式）——**读与写两个方向都剥**：编辑保存后本地库仍是规范 id，
  否则再读一层前缀就会让该包永远导不出去。编辑既有条目时描述框之外的块
  （标题 / 列表等）原样保留。
...
| `flutter test` | **1628 通过 / 6 跳过**（3 条 golden 默认跳过 + 3 条依赖 `--dart-define` 私有包路径的用例；跳过点与上一基线同一组） |
