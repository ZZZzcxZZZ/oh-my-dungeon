# 统一战役、规则构筑与 Actor Markdown 实施计划

**规格：** `docs/superpowers/specs/2026-07-28-unified-campaign-rules-and-actor-markdown-design.md`
...
4. 增加四种身份切换与临时身份不落 Actor 的 Widget/E2E 测试。
...
## Phase 4：Actor Markdown 与模板实例

1. 定义 `ActorDocument`、`ActorKind` 和模板引用模型。
2. 先写 Markdown 解析、校验、往返和错误提示测试。
3. 实现 `dnd-table-actor/v1` 解析器与导出器。
4. 角色库增加 Markdown 导入预览与导出入口。
5. 角色卡外壳支持 player/npc/monster/companion 专属区块。
6. 从怪物模板创建可编辑本地 Actor，并可发布到战役。
7. 物品模板授予后生成独立物品实例。
