...
**架构：** 保留当前 NestJS Service 与 Flutter Repository/Controller，不引入通用命令总线。数据库当前状态仍是真相；每个已验证的角色操作在同一事务中更新状态并追加不可变 `GameEvent`。旧 `Character.data`、`CampaignCharacter.sheetJson` 和 Drift 数据由版本化适配器兼容读取，逐条迁移写路径。
...
`GameEvent(schemaVersion, type, campaignId?, characterId?, characterType,
characterId, requestId, targets, cause, before, after, payload, occurredAt)`。
...
每个方法执行权限、revision、D&D 边界、事务状态更新和事件写入。调用方不能提交任意 `before`、`after` 或 character 身份。
