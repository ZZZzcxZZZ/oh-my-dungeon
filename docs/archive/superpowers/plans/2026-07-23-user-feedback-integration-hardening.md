...
| 4 | 临时角色应"用后即弃"，不保存到角色切换的临时角色栏 | 临时角色目前作为 `CampaignCharacter lifecycle='temporary'` 持久化在服务器，并展示在角色面板"临时角色"分区 | P7 |
...
- `character.hp_changed`、`character.item_granted`
...
- 三处实例化接线：
  ...
  - `campaign_character_sheet_launcher.dart:170`：sink 同上，附带 `characterId`
...
- 权限：自己可编辑自己；DM 可编辑任意 Character；其他玩家只读
...
- **批量扣血/治疗**：选择多个 Character + 输入数值（正数治疗/负数伤害）→ 服务端原子更新 + 广播 `character.hp_changed` 系统事件
- **给予装备**：从资料库选物品 → 写入 Character inventory + 广播 `character.item_granted` 系统事件
- **快速检定**：选 Character + 检定类型 + DC → DM 代掷 + 广播 `roll.check` 事件
...
- 重构 `campaign_chat_page.dart:1058-1062` `_TemporaryIdentityDraft`：
  - 草稿保留在内存（现状）
  - **不再调用服务端创建 `CampaignCharacter`**，而是把 displayName 作为 speaker snapshot 写入消息
- 服务端 `sendMessage` 接受可选 `speakerSnapshot: {displayName, avatarUrl?}`，不再创建临时 Characters
- 角色面板 `campaign_characters_panel.dart:59` 删除"临时角色"分区
- 历史消息回读时直接展示 snapshot 中的 displayName
- 同步清理 `campaign_character_controller.dart:334-357` `createTemporaryNpc` 与服务端 `sendDraftCharacterMessage`（`campaigns.service.ts:617-709`）
- **验证**：e2e test 验证临时身份不创建 Character；消息发送后草稿清空
...
| `CharacterCampaignBinding` 全套 | 服务端 `characters.service.ts:121-168,193-205,230-...` + 客户端 `character_api_client.dart:57,64,211,227,233,249` + Prisma `schema.prisma:517` | Wave 3（CampaignCharacter 体系完全接管后） | 删除表与端点，迁移存量数据到 CampaignCharacter |
| 临时角色持久化路径 | 服务端 `sendDraftCharacterMessage`（`campaigns.service.ts:617-709`）+ 客户端 `createTemporaryNpc`（`campaign_character_controller.dart:334-357`） | Wave 5（Task 5.2） | 改为 speaker snapshot，不落库 |
...
4. **Wave 4 在 Wave 3 之后**：档案 wiki 化的「时间格式化与编辑者署名」需要 `CampaignCharacterAudit` 与 `CampaignEvent` 已稳定；标签筛选也需要服务端查询能力（不依赖 CampaignEvent，但依赖 `campaigns.service.ts` 拆分）。
