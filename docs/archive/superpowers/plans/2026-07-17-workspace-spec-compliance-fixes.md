...
| 8 | 创建角色失败 | — | 待复现错误日志后定位。最可能原因：① DM 走 `/characters/publish` 接口尝试创建 NPC（被服务端 400 拒）；② 重复 publish 同一 sourceCharacterId 时 baseRevision 冲突 409；③ sheetJson 含 null/数组导致 400 | 待定位 |
| 9 | DM 模式下角色创建只能创建临时角色 | §DM 角色生命周期：常驻（NPC/怪物/同伴）、临时、已归档；DM 可在队伍页面完整管理 | `CharactersTabPage` DM 模式直接显示 `CampaignCharacterDirectoryPage` 而非本地角色创建入口；DM 创建常驻 NPC 的入口缺失；只有 chat 页的"快速临时身份"路径可创建临时角色 | 偏差 |
...
- `CharactersTabPage`：DM 模式下不应直接替换为 `CampaignCharacterDirectoryPage`；本地角色创建入口对所有人可用，战役角色管理按 capabilities 显示。
...
- 验证：`flutter test test/campaign_chat_character_test.dart` 全绿。
...
- 旁白 / DM
- 账号场外身份
- 常驻 NPC、怪物、同伴（从 workspace.characters 列表）
- 临时角色（草稿）
...
**步骤**：
1. 在 `PublishCharacterSheet` 与 `CampaignCharacterController` 的 catch 块里把服务端错误体（`error.message`）显示在 SnackBar，让用户看到具体原因。
...
4. 修复重复 publish 时 baseRevision 处理：在 `CampaignCharacterController` 自动重读最新 revision 后重试一次。
...
**改动**：
- 移除 `AvatarPicker` 嵌入。
- 发布时若 `character.avatarUrl` 是本地路径，调用媒体上传接口得到 assetId，写入 character sheet。
- `onPickImage` 参数移除。
...
**目标**：按规范 §DM 角色生命周期完整管理，DM 可在 `战役中心 → 队伍` 创建常驻 NPC/怪物/同伴（characterType: npc/unclaimed/companion，lifecycle: persistent）。

**改动**：
- `CampaignTeamPanel` DM 区加"创建常驻角色"按钮。
- 弹出表单：名称、characterType 选择（npc/unclaimed/companion）、头像、初始 HP、生命周期（默认 persistent）。
- 调用 `POST /campaigns/:id/characters` 接口（`campaignCharacterController.create`）。
...
**目标**：DM 模式下"角色"tab 仍可创建本地角色（用于 DM 自己的玩家角色或备选角色），不应被 CampaignCharacterDirectoryPage 完全替代。
