...
void main() {
  group('class content navigation', () {
    testWidgets(
      'subclass and classFeature are independently searchable by type',
      (tester) async {
        ...
        expect(controller.results.map((e) => e.id), [
    ...
    testWidgets(
      'class detail shows only this class features grouped by level',
      (tester) async {
        final repository = MemoryContentRepository(
          initialEntries: _fighterPackage(),
        );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.descendant(of: dialogFinder, matching: find.text('等级特性')),
          findsOneWidget,
        );
        expect(
    ...
    testWidgets('back restores the previous class scroll position', (
      tester,
    ) async {
      ...
      // 顶部特性初始可见.
      expect(
        find.descendant(of: find.byType(Dialog), matching: find.text('回气')),
        findsOneWidget,
      );
