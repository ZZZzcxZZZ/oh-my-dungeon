...
///
/// **接线**：写入方是 `RulesDrivenCharacterBuilder`（派生时把
/// `ResolvedClassRules.fieldSources` 落库）；角色自身的条目身份来源由
/// `CharacterRuleProjector` 随再派生刷新；读取方是角色页（任务 9）与导入报告 /
/// 预览（任务 11）。序列化与排序口径只在这里，接入方不得另写一份
/// `{originId, tier}` 解析。
