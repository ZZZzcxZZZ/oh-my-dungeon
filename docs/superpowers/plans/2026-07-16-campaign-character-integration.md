...
- Validate target actor and check request payload.
- Preserve server-derived actor display snapshots.

**Client**

- Add typed message helpers and event-data serialization.
- Render actor identity for both speech and actions.
- Render system events in bold and roll/check request events with focused
  Material 3 surfaces.
- Allow an actor owner to answer a check request and persist the linked roll.
...
- Put information, actors, shared content, and owner controls in the hub.
- Let the owner open an actor, select an ability/save/skill, and send a check
  request.
