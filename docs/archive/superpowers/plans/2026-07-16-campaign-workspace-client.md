...
- 修改：`apps/client_flutter/lib/src/features/campaigns/domain/campaign_character.dart`
...
- 新建：`apps/client_flutter/lib/src/features/campaigns/presentation/widgets/campaign_character_quick_sheet.dart`
...
4. quick sheet 用 ModalBottomSheet 呈现，玩家自己打开本地/战役角色详情，DM 可进入 campaign character sheet。
5. 运行 widget 测试至绿。
...
2. 运行 `flutter test test/campaign_chat_character_test.dart`，确认失败。
...
4. 发布/绑定成功后刷新 campaign context，并让当前 identity 自动切到 bound character。
