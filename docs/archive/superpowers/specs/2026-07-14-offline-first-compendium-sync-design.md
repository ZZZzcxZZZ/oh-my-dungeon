...
9. 玩家将本地角色发布到战役后形成 `CampaignCharacter`。DM 对 `CampaignCharacter` 拥有完整编辑权，修改必须有审计记录并同步回玩家本地角色。
...
- `campaign_character_cache(id, campaignId, ownerUserId, characterJson, revision, syncState)`
...
聊天室消息只引用 `campaignCharacterId`，显示信息来自战役 Character 的当前缓存。

### 9.4 角色同步和冲突

- 本地角色和战役 Character 均有单调递增 `revision`。
- 上传必须携带 `baseRevision`。
- 修订一致时接受写入并返回新版本。
...
DM 模式：`首页 / 战役 / 角色 / 资料库 / 设置`，但角色页变为当前战役 Character 管理，首页突出最近战役和同步异常。
...
- 引入 CampaignCharacter 完整副本和审计。
- DM 模式角色页切换为战役 Character 管理。
...
- 空资料库状态和设置页导入。
- 宽屏 Wiki 双栏与窄屏详情路由。
- Player 角色页与 DM 战役 Character 页语义不同。
- 离线状态下角色、资料和设置仍可编辑。
- 缺包角色仍能打开完整快照。
...
- Vault 数据按用户隔离。
- DM 对战役 Character 完整编辑，Player 权限受限。
- 非成员不能读取战役条目。
- Player 不能写战役条目。
...
4. `campaign-characters-content`：DM 角色管理、CampaignCharacter、战役 JSON 条目和聊天集成。
...
- Foundry VTT Characters：<https://foundryvtt.com/article/characters/>
