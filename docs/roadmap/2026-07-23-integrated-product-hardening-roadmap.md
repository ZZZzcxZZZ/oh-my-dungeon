...
1. 删除无有效任务的首页，默认进入战役。
2. “说/做”使用两个紧凑图标按钮，避免滑块选中态与点击目标混乱。
3. 战役角色栏的归档区默认折叠。
4. 修复战役中心各区块间距和视觉层级。
...
完成标准：重启后设置值不丢失；账户和服务器入口不再散落；相同控件在不同页面拥有一致尺寸、圆角和状态；任意 seed color 的亮暗主题保持可读。

### P2 资料库成为只读查阅平台

1. 从资料详情移除编辑、复制和笔记入口；导入与资料包管理只放在设置。
2. 把 `subclass`、`classFeature` 纳入类型检索；职业详情同时显示子职业和按等级分组的职业特性。
3. 所有关联条目通过同一个浮层阅读器跳转，不再混用新页面和底部小卡片。
4. 将巨大下拉框改为“筛选”按钮、底部筛选面板和少量已启用筛选标签。
5. 为职业、子职业、专长、装备、怪物和状态定义各自的可选 facet；没有结构化字段时不展示空筛选项。

完成标准：资料库不承担内容创作；任一职业可以跳转到子职业和职业特性；手机首屏搜索区不超过两行。
...
1. 档案条目增加正文 blocks、标签、关联资料和附件引用。
2. 列表复用资料库的信息架构；详情复用统一浮层阅读器。
3. 创建者和 DM 可编辑，其他成员只读；编辑记录保留更新时间和作者。
4. 线索、地点、NPC、共享规则和群文件使用同一档案容器，不分别造页面。

完成标准：条目可写完整内容、可关联、可搜索；不再使用局促的单字段上拉卡片。

### P7 会话与临时身份

1. 一个战役拥有置顶主聊天室，并可创建私聊和小群。
2. 临时身份只作为下一条消息的 speaker snapshot，不创建 `CampaignActor`。
3. 常驻 NPC 在战役角色栏管理，可被 DM 设为连续发言身份。
4. 私聊与小群仅参与者可见，战役所有权不自动授予读取私聊内容的能力。

完成标准：临时身份用后消失；角色列表只保留真正需要管理的角色；同一战役卡片可展开进入主聊、私聊和小群。

## 本轮实施边界

当前已实施 P0 的首页收敛、模式按钮、归档折叠、生命环快照/当前状态分流、消息分组偏好和 Composer 重排，并为后续阶段保留现有 API。下一批按并行开发编排执行 P1-P3 与战役中心 P0/P6 子集；P4-P5、P7 在首轮接口稳定后进入第二波，避免多个分支同时修改聊天与服务端战役核心。
