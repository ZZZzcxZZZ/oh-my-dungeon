...
void main() {
  ...
  test('loadConversations replaces the list and tracks the active campaign',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation(), directConversation(), groupConversation()],
    );
  ...
  test('loadConversations preserves the active conversation when still present',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
  ...
  test('loadConversations resets the active conversation when missing',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation(), directConversation()],
  ...
  test('loadConversations surfaces a generic message on unknown exception',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: const [],
  ...
  test('setActiveConversation updates the value and notifies listeners',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation(), directConversation()],
  ...
  test('createDirectConversation upserts and marks the new conversation active',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
  ...
  test('createDirectConversation upserts an existing conversation id', () async {
    final auth = await buildLoggedInAuthController();
    ...
    // 列表长度保持 2, 不应重复添加.
    expect(controller.conversations, hasLength(2));
    expect(controller.conversations.where((c) => c.id == 'direct-1'), hasLength(1));

    controller.dispose();
    auth.dispose();
  });

  test('createDirectConversation returns null and sets error on exception',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation()],
      createDirectException:
          const CampaignApiException('not a member', statusCode: 403),
    );
    final controller = ConversationController(
      apiBaseUrl: apiBaseUrl,
      authController: auth,
  ...
  test('createGroupConversation upserts and marks the new conversation active',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation()],
    ...
    expect(client.createGroupCalls.single.participantIds, ['user-2', 'user-3']);

    controller.dispose();
    auth.dispose();
  });

  test('createGroupConversation returns null and sets error on exception',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation()],
  ...
  test('updateConversation archives the active conversation and falls back to main',
      () async {
    final auth = await buildLoggedInAuthController();
  ...
  test('updateConversation returns null and sets error on CampaignApiException',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation(), groupConversation()],
      updateException: const CampaignApiException('forbidden', statusCode: 403),
    );
    final controller = ConversationController(
      apiBaseUrl: apiBaseUrl,
      authController: auth,
  ...
  test('mainConversation getter resolves the main kind conversation', () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [directConversation(), groupConversation(), mainConversation()],
    );
    final controller = ConversationController(
      apiBaseUrl: apiBaseUrl,
      authController: auth,
  ...
  test('directConversations and groupConversations getters sort by updatedAt desc',
      () async {
    final auth = await buildLoggedInAuthController();
    ...
    expect(
      controller.directConversations.map((c) => c.id).toList(),
      ['direct-new', 'direct-old'],
    );
    expect(
      controller.groupConversations.map((c) => c.id).toList(),
      ['group-new', 'group-old'],
    );
  ...
  test('activeConversation falls back to main when no active selection',
      () async {
    final auth = await buildLoggedInAuthController();
    final client = _RecordingCampaignClient(
      conversations: [mainConversation(), directConversation()],
